...
use memex_lib::vault::{
    self, FileContent, FileNode, VaultMeta,
};
use std::fs;
...
fn full_lifecycle_seed_to_graph_to_mutate() {
    ...
    assert!(
        adj.forward.iter().any(|(_, t)| t.iter().any(|p| p.ends_with("beta.md"))),
        "expected alpha→beta resolved edge"
    );
    assert!(
        adj.backward.iter().any(|(_, s)| s.iter().any(|p| p.ends_with("alpha.md"))),
