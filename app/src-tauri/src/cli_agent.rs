...
use crate::claude::{augmented_path, locate_bin, wait_with_timeout, CliResult, CliStatus};
...
pub fn run_prompt(
    ...
    let (bin, env) = provider_bin(provider).ok_or_else(|| format!("unknown provider: {provider}"))?;
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        // Drop either way so the child sees EOF instead of waiting.
    }
...
mod tests {
    ...
    fn gemini_args_headless_with_model() {
        let (args, stdin) =
            build_args("gemini-cli", "gemini-2.5-pro", "hello", "/tmp/x").unwrap();
        assert!(!stdin);
        assert_eq!(args[0], "-p");
