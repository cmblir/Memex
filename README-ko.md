...
**sigma.js**(WebGL) + **d3-force** 레이아웃으로 vault 전체 링크 그래프 — Obsidian이 쓰는 동일한 포스 패밀리(`forceLink` + `forceManyBody` + `forceX/Y` + 충돌)를 적용하고, 각 링크 강도를 노드 차수로 정규화해 잎 노드는 허브에 붙고 클러스터는 방사형 "민들레"로 분리됩니다. 노드는 파일, 엣지는 `[[wikilinks]]`. 허브는 링크 수에 따라 커지고, 엣지는 옅게 유지돼 노드가 구조를 드러냅니다. **링크 없는 노트(고아)까지 모든 파일이 표시**됩니다.

레이아웃은 한 번 정착한 뒤 멈춥니다(idle CPU 없음). **노드를 드래그하면 시뮬레이션이 재가열**돼 이웃이 따라오고, 놓으면 힘에 의해 제자리로 튕겨 돌아옵니다 — Obsidian과 동일. 평소엔 물리 연산이 없어 대형 vault에서도 가볍습니다.
