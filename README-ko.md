...
**three.js**(WebGL) + **d3-force-3d**로 vault 전체 링크 그래프를 **3D 우주**로 렌더링 — Obsidian이 쓰는 동일한 포스 패밀리(`forceLink` + `forceManyBody` + `forceX/Y/Z` + 충돌)를 적용하고, 각 링크 강도를 노드 차수로 정규화해 잎 노드는 허브에 붙고 클러스터는 방사형 "민들레"로 분리됩니다. 노드는 빛나는 별, 엣지는 `[[wikilinks]]`가 옅은 필라멘트로 그려집니다. 허브는 링크 수에 따라 커지고, 가까이 다가갈수록 허브부터 라벨이 나타납니다. **링크 없는 노트(고아)까지 모든 파일이 표시**됩니다.

**드래그로 카메라를 클러스터 주위로 회전**, 스크롤로 줌. 평소엔 천천히 자동 회전합니다. **별(노드)을 잡으면 시뮬레이션이 재가열**돼 이웃이 3D로 따라오고, 놓으면 제자리로 돌아옵니다. UnrealBloom 발광·depth fog·별필드로 우주 느낌을 내고, 드로어의 **밝기(Brightness)** 슬라이더로 발광 강도를 조절합니다. 앱 밖(Obsidian/Finder 편집, ingest 완료 등)에서 파일이 바뀌면 파일트리와 그래프가 **자동 갱신**됩니다.

우측 설정 드로어(톱니바퀴 아이콘)가 Obsidian 패널 구성을 따릅니다:

- **Filters** — 파일명 라이브 검색, 태그 칩, 폴더 드롭다운, *고립 노드 표시* / *존재하는 파일만* 토글.
- **Display** — *화살표*, *라벨 페이드 임계*, *노드 크기*, *링크 두께*, 그리고 **▶ 애니메이션 재생** 버튼.
- **Forces** — *중심력*, *반발력*, *링크 장력*, *링크 거리*. 각 슬라이더가 실제 d3-force 파라미터를 직접 조작합니다.
