...
Cytoscape.js + **d3-force** 레이아웃으로 vault 전체 링크 그래프 — Obsidian이 쓰는 동일한 포스 패밀리(`forceLink` + `forceManyBody` + `forceX/Y` + 충돌)를 적용합니다. 노드는 파일, 엣지는 `[[wikilinks]]`. 허브는 링크 수에 따라 커집니다. **링크 없는 노트까지 모든 파일이 표시**되며, 강한 중심 중력이 고립 노드를 클러스터 사이로 끌어모아 hairball + 바깥 링이 아니라 **하나의 조밀하고 균일한 원반**으로 정착시킵니다.
